...
class FileSaver(PersistenceInterface):
    ...
    def save(self, owner, balance):
        with open("bank_records.txt", "a") as file:
            file.write(f"Owner: {owner}, Balance: {balance:,.2f} ETB\n")
        print(f"Saved updated balance ({balance:,.2f} ETB) to file.")
...
class Account:
    ...
    def deposit(self, amount):
        if amount > 0:
            ...
            self.emailer.send_alert(self.owner, f"Deposited {amount:,.2f} ETB")
