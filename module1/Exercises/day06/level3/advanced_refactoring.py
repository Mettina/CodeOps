#  Open/Closed Principle & Factory Pattern Extension (Ethiopian Birr - ETB)
...
class SavingsAccount(Account):
    def statement(self):
        print(f"Savings Account    | Owner: {self.owner} | Balance: {self.balance:,.2f} ETB")

class CurrentAccount(Account):
    def statement(self):
        print(f"Current Account    | Owner: {self.owner} | Balance: {self.balance:,.2f} ETB")
...
class InvestmentAccount(Account):
    ...
    def statement(self):
        print(f"Investment Account | Owner: {self.owner} | Balance: {self.balance:,.2f} ETB | Risk: {self.risk_profile}")
...
class AccountFactory:
    ...
    def create(kind, owner, balance):
        ...
        # Adding support for the new investment account option here safely
...
if __name__ == "__main__":
    ...
    # Polymorphic loop handles the new type naturally without client-side changes
